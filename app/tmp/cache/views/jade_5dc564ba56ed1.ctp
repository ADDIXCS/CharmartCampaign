...
<fieldset>
  <?php
    ...
    // if(in_array($campaign['Campaign']['campaign_type'], ['lottery']) && $loginAccount['twitter_id'] == '' && $loginAccount['facebook_id'] == '')
      echo '<p class="alert alert-warning js-no-sns">このキャンペーンは、SNSアカウントからしか応募できま
  ?>
</fieldset>
せん。お客様のFacebookページか、Twitterのアカウント登録が必要です。</p>';
